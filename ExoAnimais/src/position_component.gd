...
func request_movement_to_index(index : int) -> float:
	index = clamp(index, 0, len(tile_path.path_positions))
	current_index_position = index
	
	# returns time for the movement
	return tileCharacterBody2D._on_movement_update(index)
	
func request_movement_increment(increment : int):
	var index = current_index_position + increment
	return request_movement_to_index(index)
