...
var scales : Vector2 = Vector2(max_scale, 0) #Vector2(x, y) where x + y = 1
...
func _when_scale_reaches_right():
	scales = Vector2(0, 1)
	_switch_sides()
...
func _create_dice_queue():
	...
	rps = len(dice_queue)/animation_time
...
func _process(delta):
	scales = Vector2(clamp(scales.x + delta * rps, 0, max_scale), clamp(scales.y - delta * rps, 0, max_scale))
	
	if scales.x >= max_scale:
		_when_scale_reaches_right()
	
	#TODO: fazer ele atualizar os scales
	_update_dice_scales()
